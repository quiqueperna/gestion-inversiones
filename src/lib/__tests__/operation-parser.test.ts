...
describe('parseOperationText', () => {
  ...
  it('parsea formato clave=valor', () => {
    const result = parseOperationText('simbolo=NVDA qty=5 precio=800 side=BUY fecha=2024-06-01 broker=IOL');
    expect(result).not.toBeNull();
  ...
  it('parsea formato clave=valor con nombres legacy (cantidad, tipo)', () => {
