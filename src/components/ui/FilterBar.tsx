  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap items-center gap-2 bg-zinc-900/30 p-2 rounded-xl border border-white/5">
        ...
        {DATE_OPTIONS.map(f => (
          <button key={f.id} onClick={() => onDateFilterChange(f.id)}
            className={cn(
        ...
        <div className="relative" ref={ref}>
          <button
            onClick={() => { onDateFilterChange("custom"); setShowCustom(v => !v); }}
            className={cn(
              "flex items-center gap-1 px-3 py-1 rounded-lg text-[11px] font-medium transition-all",
              dateFilter === "custom" ? "bg-blue-600 text-white" : "text-zinc-400 hover:bg-white/5"
            )}>
            Personalizado <ChevronDown className="w-3 h-3" />
          </button>
          {showCustom && (
            <div className="absolute top-full left-0 mt-2 p-4 bg-zinc-900 border border-white/10 rounded-xl shadow-2xl z-50 w-72 space-y-3">
              <div className="space-y-1">
                <label htmlFor="custom-range-start" className="text-[10px] font-bold uppercase text-zinc-500">Desde</label>
                <input id="custom-range-start" type="date" value={customRange.start}
                  onChange={e => onCustomRangeChange({ ...customRange, start: e.target.value })}
                  className="w-full bg-black border border-white/10 rounded-lg p-2 text-xs outline-none focus:border-blue-500 text-zinc-200" />
              </div>
              <div className="space-y-1">
                <label htmlFor="custom-range-end" className="text-[10px] font-bold uppercase text-zinc-500">Hasta</label>
                <input id="custom-range-end" type="date" value={customRange.end}
                  onChange={e => onCustomRangeChange({ ...customRange, end: e.target.value })}
                  className="w-full bg-black border border-white/10 rounded-lg p-2 text-xs outline-none focus:border-blue-500 text-zinc-200" />
              </div>
              <button onClick={() => setShowCustom(false)}
                className="w-full bg-blue-600 hover:bg-blue-500 py-1.5 rounded-lg text-[11px] font-bold uppercase transition-all">
                Aplicar
              </button>
            </div>
          )}
