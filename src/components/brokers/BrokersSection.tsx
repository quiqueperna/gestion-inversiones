...
import { Broker } from "@/lib/data-loader";
...
export default function BrokersSection({ brokers, onAdd, onRemove, onEdit }: BrokersSectionProps) {
    ...
    <div className="space-y-4">
      <div className="bg-zinc-900/50 rounded-[8px] border border-white/10 overflow-hidden">
        <div className="p-3 bg-zinc-900 border-b border-white/10">
          <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-zinc-400">Gestión de Brokers</h3>
        </div>
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-white/5 text-[13px]">
            {brokers.map(b => (
              <tr key={b.id} className="hover:bg-white/[0.03] transition-colors h-9">
                <td className="py-1 px-4 text-zinc-600 text-[11px]">#{b.id}</td>
                ...
                <td className="py-1 px-4 text-right">
                  {editingId === b.id ? (
                    ...
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => startEdit(b)} className="p-1 text-zinc-600 hover:text-blue-400 transition-colors">
                        <Pencil className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => onRemove(b.id)} className="p-1 text-zinc-600 hover:text-red-400 transition-colors">
      ...
      <div className="bg-zinc-900/50 rounded-[8px] border border-white/10 p-4">
        <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-3">Agregar Broker</p>
        <div className="flex gap-2">
          <input
            value={nombre}
            onChange={e => setNombre(e.target.value)}
            placeholder="Nombre (ej: Vanguard)"
